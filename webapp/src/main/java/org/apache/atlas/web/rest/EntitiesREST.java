...
public class EntitiesREST {
    ...
    @Inject
    public EntitiesREST(AtlasEntityStore entitiesStore, AtlasTypeRegistry atlasTypeRegistry, MetadataService metadataService, AtlasInstanceRestAdapters restAdapters) {
    ...
    private SearchFilter getSearchFilter() {
        ...
        if (null != httpServletRequest && null != httpServletRequest.getParameterMap()) {
            for (Map.Entry<String, String[]> entry : httpServletRequest.getParameterMap().entrySet()) {
                searchFilter.setParam(entry.getKey(), entry.getValue()[0]);
            }
        }
        return searchFilter;
    }
